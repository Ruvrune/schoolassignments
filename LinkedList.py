class LinkedList:
    ...
    def __str__(self):
        ...
        return result
 # Clear the list - Garbage collectionen i Python tar seg visst av dette. Usikker på om dette er riktig?
    def clear(self):
    ...
    def remove(self, e):
        if self.__size == 0:
            ...
            temp = self.__head
            #Hvis elementet er i starten av lista
            if temp.element == e:
                self.removeFirst()
            else:
                for i in range(self.__size):
                    #Hvis oppdaget element er sist i lista
                    if temp.element == e:
                        if i == self.__size - 1:
                            self.removeLast()
                            return True
